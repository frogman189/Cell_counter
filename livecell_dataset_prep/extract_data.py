...
def save_image_and_masks_from_train_json(
    ...
    Image.fromarray(binary_mask*255).save(os.path.join(output_dir, f"{image_name}_mask.png"))

    # per-instance
    for i, ann in enumerate(anns):
        m = coco.annToMask(ann).astype(np.uint8) * 255
        Image.fromarray(m).save(os.path.join(output_dir, "cell_masks", f"cell_{i}.png"))

    # copy original image using coco path
    rel_path = matched_img['file_name']
    src_img_path = os.path.join(image_src_dir, rel_path)
    dst_img_path = os.path.join(output_dir, os.path.basename(rel_path))
    os.makedirs(os.path.dirname(dst_img_path), exist_ok=True)
    shutil.copyfile(src_img_path, dst_img_path)
    print(f"✅ Saved {len(anns)} cell masks and image to: {output_dir}")
